...
#################################################################################################
ground_truth_fig = plt.figure()
ax = ground_truth_fig.add_subplot(111, projection='3d')
...
# Set initial values.
p_est[0] = ground_truth.p[0]
v_est[0] = ground_truth.v[0]
q_est[0] = Quaternion(euler=ground_truth.r[0]).to_numpy()
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    K = p_cov_check.dot(h_jac.T).dot(np.linalg.inv(h_jac.dot(p_cov_check).dot(h_jac.T) + sensor_var * np.identity(3)))
    ...
    q_hat = Quaternion(euler = error[6:9]).quat_mult_right(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    rotation = Quaternion(*q_est[k - 1]).to_mat()
    
    # 1. Update state with IMU inputs
#    p_est[k] = p_est[k - 1] + delta_t * v_est[k - 1] + delta_t**2 * (rotation.dot(imu_f.data[k - 1]) + g) / 2
#    v_est[k] = v_est[k - 1] + delta_t * (rotation.dot(imu_f.data[k - 1]) + g)
#    q_est[k] = Quaternion(axis_angle=imu_w.data[k - 1] * delta_t).quat_mult_right(q_est[k - 1])
#
    p_est[k] = p_est[k-1] + delta_t*v_est[k-1] + (delta_t**2 / 2)*(rotation.dot(imu_f.data[k-1]) + g)
    v_est[k] = v_est[k-1] + delta_t*(rotation.dot(imu_f.data[k-1]) + g)
    q_est[k] = Quaternion(axis_angle=imu_w.data[k-1] * delta_t).quat_mult_right(q_est[k-1])
    ...
    F[3:6, 6:] = -(rotation.dot(skew_symmetric(imu_f.data[k-1].reshape((3,1))))) 
    ...
    while gnss_i < gnss.t.shape[0] and imu_f.t[k] >= gnss.t[gnss_i]:
...
for i in range(3):
    ax[0, i].plot(range(num_gt), ground_truth.p[:, i] - p_est[:num_gt, i])
...
for i in range(3):
    ax[1, i].plot(range(num_gt), \
        angle_normalize(ground_truth.r[:, i] - p_est_euler[:num_gt, i]))

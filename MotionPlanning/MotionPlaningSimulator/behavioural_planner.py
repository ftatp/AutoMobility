...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            if stop_sign_found:
                print("Follow Lane, Stop sign Exists True")
        ...
        # the stop sign and transition to the next state.
            ...
            if self._stop_count == STOP_COUNTS:
                ...
                self._goal_index = goal_index
                self._goal_state = waypoints[goal_index]
                print("stay stopped. Stop sign exists?", stop_sign_found)
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            ...
            arc_length += np.sqrt(np.sum(np.square(np.array(waypoints[wp_index + 1][:2]) - np.array(waypoints[wp_index][:2]))))
            wp_index += 1
...
def get_closest_index(waypoints, ego_state):
    ...
    closest_index = 0
    pos = [ego_state[0], ego_state[1]]

    for i in range(len(waypoints)):
        waypoint = waypoints[i]
        dist = np.sqrt(np.sum(np.square(np.array(waypoint[:2]) - np.array([pos]))))
        #print("Closest Len : %f"%closest_len)
        #print("Dist : %f"%dist)

        if closest_len > dist:
            closest_len = dist
            closest_index = i
            #print("Found more close way point than before")

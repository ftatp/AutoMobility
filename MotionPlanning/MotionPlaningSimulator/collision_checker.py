    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # score = np.sqrt( np.sum( (np.array(goal_state[:2]) - np.array([paths[i][0][center_idx], paths[i][1][center_idx]]) )**2 ) )
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            score += self._weight * np.sqrt( np.sum( (np.array([paths[j][0][-1], paths[j][1][-1]]) - np.array([paths[i][0][-1], paths[i][1][-1]] ))**2 ) )

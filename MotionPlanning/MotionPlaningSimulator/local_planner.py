...
class LocalPlanner:
    ...
    def get_goal_state_set(self, goal_index, goal_state, waypoints, ego_state):
        ...
        delta_xy = np.array(waypoints[idx + 1][:2]) - np.array(waypoints[idx][:2])
        heading = np.arctan2(delta_xy[1], delta_xy[0])
        ...
        to_local_coord_mat = np.array([
            [cos(ego_state[2]), sin(ego_state[2])],
            [-sin(ego_state[2]), cos(ego_state[2])],
        ])

        goal_local_xy = np.dot(to_local_coord_mat, np.array(goal_state_local[:2]))

        goal_x = goal_local_xy[0]
        goal_y = goal_local_xy[1]
        ...
        for i in range(self._num_paths):
            ...
            x_offset = offset * cos(pi / 2 + goal_t)
            y_offset = offset * sin(pi / 2 + goal_t)
